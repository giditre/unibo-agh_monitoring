...
def ewma(alpha, sample, previous):
  if previous == None:
    return sample
  else:
    return sample*alpha + (1.0-alpha)*previous
...
try:
        ...
        if time.time() > timestamp + 1:
            ...
            for x in flows_dict:
                ...
                entry = {
                  ...
                  'perSecond': flows_dict[x].perSecond * 8 * sampling_ratio,
                  'ewmaDatarate': flows_dict[x].ewma * 8 * sampling_ratio if flows_dict[x].ewma else 0,
    ...
    #for x in flows_dict:
        ...
        #if flows_dict[x].lastTime != flows_dict[x].firstTime:
            #bytes = flows_dict[x].frameCounter * 8 * sampling_ratio
