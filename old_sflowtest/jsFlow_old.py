                    ...
                    if sFlowData.sample[i].record[j].sampleType == 1:
                        if sFlowData.sample[i].record[j].format == 1 and sFlowData.sample[i].record[j].enterprise == 0:
                            ...
                            if is_existing_entry:
                              ...
                              flows_dict[k].entryTime = sampleTimeStamp

                              if sampleTimeStamp in flows_dict[k].perSecond:
                                flows_dict[k].perSecond[sampleTimeStamp] += record.frameLength
                              else:
                                flows_dict[k].perSecond[sampleTimeStamp] = record.frameLength
        ...
        if time.time() > timestamp + 1:
            ...
            for x in flows_dict:
                ...
                entry = {
                  ...
                  'entryTime': flows_dict[x].entryTime,
                  'frameCounter': flows_dict[x].frameCounter,
                  'perSecond': flows_dict[x].perSecond,
