...
tshark -r $fname -d udp.port==16343,sflow -Y udp -T fields -E separator=' ' -e frame.len -e ip.len -e udp.length 2>/dev/null | while true; do
  if read line; then
    ...
    printf_err "\n"
    echo "N $count eth $ethsum ip $ipsum udp $udpsum"
    break
  fi
done
