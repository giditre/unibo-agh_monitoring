...
class FlowInfo:
    def __init__(self, timeStampFirst, length, srcIP, dstIP):
        ...
        self.perSecond = [0] * windowEWMA
        self.perSecondEWMA = 0
        # find a solution for values timeout
                        ...
                        if sFlowData.sample[i].record[j].format == 1 and sFlowData.sample[i].record[j].enterprise == 0:
                            ...
                            if is_existing_entry:
                              ...
                              if sampleTimeStamp > flows_dict[k].lastTime:
                                  flows_dict[k].lastTime = sampleTimeStamp
                                  flows_dict[k].perSecond.pop(0)
                                  flows_dict[k].perSecond.append(record.frameLength * 8 * sample_rate)
                              elif sampleTimeStamp == flows_dict[k].lastTime:
                                  flows_dict[k].perSecond[-1] += record.frameLength * 8 * sample_rate
                              else:
                                  sys.exit("How did we get here? sampleTimeStamp is smaller than flows_dict[k].lastTime. Is time going backwards?")
                              
                              #print("perSecond", flows_dict[k].perSecond)

                              ewma_vector = numpy_ewma_vectorized_v2(np.array(flows_dict[k].perSecond).astype("float"), windowEWMA)
                              #print("perSecondEWMA", ewma_vector)
                              flows_dict[k].perSecondEWMA = int(ewma_vector[-1])
                              print(flows_dict[k].perSecondEWMA)
        ...
        if time.time() > timestamp + 1:
            ...
            for x in flows_dict:
                ...
                if flows_dict[x].lastTime != flows_dict[x].firstTime:
                    data = flows_dict[x].frameCounter * 8 * sample_rate
                ...
                entry = {
                  ...
                  'perSecond': flows_dict[x].perSecondEWMA,
            ...
            flows_json['timestamp'] = timestamp
