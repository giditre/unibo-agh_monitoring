...
class FlowInfo:
    def __init__(self, timeStampFirst, length, srcIP, dstIP):
        ...
        self.srcIP = srcIP
        self.dstIP = dstIP
        self.timeout = time.time() #find a solution for values timeout
...
try:
    while True:
        ...
        for i in range(sFlowData.NumberSample):
            if sFlowData.sample[i].sampleType == 1:
                sample_rate = sFlowData.sample[i].sampleRate
                        ...
                        if sFlowData.sample[i].record[j].format == 1 and sFlowData.sample[i].record[j].enterprise == 0:
                            ...
                            for k in flows_dict:
                                ...
                                if k.inputPort == sFlowData.sample[i].inputInterface and k.outputPort == sFlowData.sample[i].outputInterface:
                                    ...
                                    #solution for values timeout
                                    if time.time() > flows_dict[k].timeout + 5:
                                        flows_dict[k].firstTime = sFlowData.sysUpTime
                                        flows_dict[k].lastTime = sFlowData.sysUpTime
                                        flows_dict[k].frameCounter = 0
                                    
                                    if sFlowData.sysUpTime > flows_dict[k].lastTime:
                                        flows_dict[k].lastTime = sFlowData.sysUpTime
                                    
                                    flows_dict[k].frameCounter = flows_dict[k].frameCounter + record.frameLength
                                    boolean = 1
                                    
                                    flows_dict[k].timeout = time.time()#solution for values timeout
                                
                            if boolean == 0:
                                x = Port(sFlowData.sample[i].inputInterface, sFlowData.sample[i].outputInterface)
                                y = FlowInfo(sFlowData.sysUpTime, record.frameLength, record.srcIP, record.dstIP)
                                flows_dict[x] = y
        #writing 2 file: text and json ... removing text file
        if time.time() > timestamp + 1:
            ...
            for x in flows_dict:
                ...
                entry['inputPort'] = x.inputPort
                
                entry['outputPort'] = x.outputPort
                
                entry['srcIP'] = flows_dict[x].srcIP
                
                entry['dstIP'] = flows_dict[x].dstIP
                
                entry['firstTime'] = flows_dict[x].firstTime
                
                entry['lastTime'] = flows_dict[x].lastTime
                
                entry['frameCounter'] = flows_dict[x].frameCounter
                if flows_dict[x].lastTime != flows_dict[x].firstTime:
                    bytes = flows_dict[x].frameCounter * 8 * sample_rate
                    interval = ( flows_dict[x].lastTime - flows_dict[x].firstTime ) / 1000
                    band = bytes / float(interval)
                else:
                    band = 0
                
                entry['bandwidth'] = band
    ...
    #for x in flows_dict:
        ...
        #print "Port: %d" % x.inputPort
        #print ">output port: %d" % x.outputPort
        #print ">srcIP: " + flows_dict[x].srcIP
        #print ">dstIP: " + flows_dict[x].dstIP
        #print ">first time: %d" % flows_dict[x].firstTime
        #print ">last time: %d" % flows_dict[x].lastTime
        #print ">frame counter: %d" % flows_dict[x].frameCounter
        #if flows_dict[x].lastTime != flows_dict[x].firstTime:
            #bytes = flows_dict[x].frameCounter * 8 * sample_rate
            #interval = ( flows_dict[x].lastTime - flows_dict[x].firstTime ) / 1000
            #band = bytes / float(interval)
