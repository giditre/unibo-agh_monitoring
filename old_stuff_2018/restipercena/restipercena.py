"""
Simple HTTP server to invoke iperf on the machine
"""
...
class iperfThread(threading.Thread):
    ...
    def __init__(self, params):
      ...
      for pair in params.split('&'):
        ...
        self.cmd += ' -{} {}'.format(key, value)
      # define output file name
      self.f_name = 'outiperf{}.txt'.format(round(time()*1000))
      print('cmd: {} - out: {}'.format(self.cmd, self.f_name))

    def run(self):
      #proc = subprocess.Popen(self.cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1, universal_newlines=True)
      proc = subprocess.Popen(self.cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
      #tio = TextIOWrapper(proc.stdout, encoding="utf-8")
      with open(self.f_name, 'w') as f:
        while True:
          ...
          #out = tio.readline()
          #print('out', out)
          #print('stdout', self.f_name, proc.stdout.read(16))
          #print('stderr', self.f_name, proc.stderr.read(16))
...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith('/iperf'):
          print(self.path)
          
          thread = iperfThread(self.path.replace('/iperf?',''))

          f_name = thread.get_f_name()

          f = BytesIO()
          f.write('{}\n'.format(f_name).encode('utf-8'))
...
class CustomHTTPServer(http.server.HTTPServer):
    ...
    def __init__(self, address, handlerClass=SimpleHTTPRequestHandler):
        ...
        print('HTTP server {} at http://{}:{}'.format(self.server_info, address[0], address[1]),  end='\n\n')
...
if __name__ == '__main__':
  ...
  #parser.add_argument("ip", help="server IP address", nargs='?', default='0.0.0.0')
  ...
  server = CustomHTTPServer(('0.0.0.0', int(args.port)))
