...
tshark -r $fname -d udp.port==16343,sflow -Y 'sflow && sflow_245.sampletype==1' -T fields -E separator=' ' -e frame.len -e sflow_245.numsamples -e ip.len -e sflow_5.sample_length -e sflow_245.sequence_number -e sflow_245.sampletype 2>/dev/null | python3.6 <(
cat <<-EOF
import sys
import json
