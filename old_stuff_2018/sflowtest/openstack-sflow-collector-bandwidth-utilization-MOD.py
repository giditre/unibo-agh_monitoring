...
try:
                ...
                for j in range(sFlowData.sample[i].recordCount):
                    if sFlowData.sample[i].record[j].sampleType == 1:
                        if sFlowData.sample[i].record[j].format == 1 and sFlowData.sample[i].record[j].enterprise == 0:
                            ...
                            # IMPLEMENTATION: -flows_dict: matching key by port, or new entry
                            boolean = 0
                            for k in flows_dict:
                                #print "...checking dict..."
                            ...
                            if boolean == 0:
                                ...
                                flows_dict[x] = y
        #writing a file .....EDIT: write the file every sec
        if time.time() > timestamp + 1:
            stdout = sys.stdout
            sys.stdout = open("sflow-temp.txt", "w")
            print ""
            print "Flows Dictionary:\n"
            for x in flows_dict:
                print "---------------------------------------------"
    ...
    print ""
    #print "Flows Dictionary length: %d" % len(flows_dict)
    #print ""
    #print "------dict------"
    #for x in flows_dict:
        #print "Connection: " + x.srcIP + " to " + x.dstIP
        #print "Port: %d" % x.inputPort
        #print ">output port: %d" % x.outputPort
        #print ">first time: %d" % flows_dict[x].firstTime
        #print ">last time: %d" % flows_dict[x].lastTime
