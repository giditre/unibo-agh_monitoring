...
#class RyuAPI():
#  def __init__(self):
#    self.json_vnf1 = json.loads("""{
#      "dpid": 1,
#      "match": {
#        "in_port": 1,
#        "eth_type": 2048,
#        "ipv4_src": "10.1.4.1/32",
#        "ipv4_dst": "10.1.4.40/32"
#      },
#      "actions": [
#        {
#          "type": "SET_FIELD",
#          "field": "eth_dst",
#          "value": "46:3a:b0:20:6f:66"
#        },
#        {
#          "type": "OUTPUT",
#          "port": "11"
...
#    }""")
#
#    #print json.dumps(self.json_vnf1, indent=2)
#    #print json.dumps(self.json_vnf2, indent=2)
#
#    self.vnf1_full = False 
#    self.vnf2_full = False
#
#  def set_vnf1_full(self):
#    if not self.vnf1_full:
#      self.vnf1_full = True
#      print "Send flow rule change request to SDN controller"
#      #url = "http://{}:{}/stats/flowentry/{}".format(ctrl_addr, ctrl_port, entry_action)
#      url = "http://127.0.0.1:8080/stats/flowentry/add"
#      self.json_vnf2["match"]["ipv4_dst"] = "10.1.4.42/32"
#      r = requests.post(url, data=json.dumps(self.json_vnf2))
#      print "Response code: {}".format(r.status_code)
#
#  def set_vnf2_full(self):
#    self.vnf2_full = True
                      ...
                      if sFlowData.sample[i].record[j].sampleType == 1:
                          if sFlowData.sample[i].record[j].format == 1 and sFlowData.sample[i].record[j].enterprise == 0:
                              ...
                              for k in flows_dict:
                                  #print "...checking dict..."
                                  #print "k", k
                                  #print "s", sFlowData.sample[i]
                                  #print "r", record
                                  #print flows_dict[k].dstIP
                                  #print record.dstIP
                                  if k.inputPort == sFlowData.sample[i].inputInterface and k.outputPort == sFlowData.sample[i].outputInterface and flows_dict[k].srcIP == record.srcIP and flows_dict[k].dstIP == record.dstIP:
                              ...
                              if is_existing_entry:
                                ...
                                if sampleTimeStamp not in flows_dict[k].shortTermDataPerSecond:
                                  flows_dict[k].shortTermDataPerSecond[sampleTimeStamp] = record.frameLength*8*sampling_ratio
                                else:
                                  flows_dict[k].shortTermDataPerSecond[sampleTimeStamp] += record.frameLength*8*sampling_ratio
          ...
          if time.time() > timestamp + 1:
              ...
              for x in flows_dict:
                  ...
                  entry = {
                    ...
                    'ewmaDatarate': int(flows_dict[x].ewma) if flows_dict[x].ewma != None else 0,
              ...
              # 
              #for flow in flows_json["flows"]:
              #  if flow["srcIP"] == "10.1.4.1" and flow["ewmaDatarate"] > THRESHOLD:
              #    print flow["srcIP"], flow["dstIP"], flow["inputPort"], flow["outputPort"], flow["ewmaDatarate"]

              # # JNCA revision
              # # sum all traffic on port 44 (the port of vnf1) and compare it to the threshold
              # vnf1_load = 0
              # vnf1_intf = 44
              # vnf2_load = 0
              # vnf2_intf = 48
              # for flow in flows_json["flows"]:
              #   if flow["outputPort"] == vnf1_intf:
              #     vnf1_load += flow["ewmaDatarate"]
              #   if flow["outputPort"] == vnf2_intf:
              #     vnf2_load += flow["ewmaDatarate"]
              # print "VNF1 load", vnf1_load
              # print "VNF2 load", vnf2_load
              # # when it exceeds the threshold, interact with the SDN controller
              # # which in this PoC means send REST requests to Ryu to configure steering of next flows towards VNF2
              # if vnf1_load > THRESHOLD:
              #   print "VNF1 load exceeds THRESHOLD"
              #   ryu_api.set_vnf1_full()
              # # I plan on sending 30Mbps flows with THRESHOLD at 50Mbps, so the flows to .40 and .41 will go through VNF1 and from .42 will have to go through VNF2
              # # sending the command from here actually emulates the monitoring module suggesting this steering to the SDN controller, and the SDN controller acting proactively before the next flow comes
